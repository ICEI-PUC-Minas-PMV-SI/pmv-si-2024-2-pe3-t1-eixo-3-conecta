...
function addFeedbackItem(name, date, text) {
  ...
  const dateToLocale = dateObj.toLocaleDateString("pt-BR", {
    month: "long",
    year: "numeric",
  });
  const formattedDate = dateToLocale.replace(" de ", ", ");
  feedbackDate.innerText =
...
const populateCandidateData = async () => {
  ...
  tasks.forEach((task) => {
    ...
    verticalTaskCard.addres =
      candidateData.street + ", " + candidateData.number;
