...
async function countRegistrationsByCpf(cpf) {
  try {
    ...
    const approvedCandidates = candidates.filter(
      (candidate) =>
        candidate.status.toUpperCase() === "APROVADO" && candidate.cpf === cpf
    );
    const pendingCandidate = candidates.filter(
      (candidate) =>
        candidate.status.toUpperCase() === "PENDENTE" &&
        countPendingActive(candidate.timestamp) > dataAtual
    );
